...
def load_data():
    ...
    dataframe = pd.read_csv('point/userpoint.csv', index_col=None)
    userid = list(dataframe['id'])
    userpoint = list(dataframe['point'])
    usercons = list(dataframe['constraint'])
    return symbols, prices_out, caps_out, date_out, userid, userpoint, usercons
...
def BL_model():
    ...
    def BL(names, prices, caps, userid, userpoint, usercons):
        n = len(names)
        ...
        # 计算用户资产配置权重
        res = optimize_frontier(Pi_adj + rf, C, rf, usercons)
    ...
    names, prices, caps, dates, userid, userpoints, usercons = load_data()
    num = len(userid)
    userWeight = BL(names, prices, caps, userid[0], userpoints[0], usercons[0])
    for i in range(1, num):
        res = BL(names, prices, caps, userid[i], userpoints[i], usercons[i])
